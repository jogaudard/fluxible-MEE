...
![](figure_1a.jpg){#fig-soil fig-align="left" width=98%}

![](figure_1b.jpg){#fig-threed fig-align="right" width=98%}


![](figure_1c.jpg){#fig-tent fig-align="center" width=200%}
...
The latter approach has the advantage of being easier to implement and optimise when fitting a model to real data.
...
![The `fluxible` R package processes raw field measured gas concentration data into analysis-ready fluxes in five steps, including data quality assessment. All functions have the `flux_` prefix. A list of all functions is available in Appendix F.](figure_2.png){#fig-concept}
...
The gas concentration dataset is produced by a logger or a gas analyser and contains at minimum a datetime column and a gas concentration column for all the measurements (i. e. a time series of gas concentration that will result in a single flux).
...
It was decided here to strictly apply the HM model for comparison purpose, as it is the closest to the SFP exponential workflow, but this strongly supports our recommendation to use the model presented in @zhaoCalculationDaytimeCO22018 (i. e. `exp_zhao18` or `exp_tz`).
...
Further developments of `fluxible` include (1) a segmentation tool to automatically select the focus window, based on residuals or on environmental variables, and (2) extending the workflow to transform the concentration in mole per volume before fitting a model, which would account for temperature changes during the measurements better.
